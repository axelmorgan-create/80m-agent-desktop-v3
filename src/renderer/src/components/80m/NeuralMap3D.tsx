...
interface NeuralNode3D { id: string; label: string; value: number; }
interface GraphNote { id: string; path: string; name: string; relativePath: string; linkCount: number; }
...
function sphericalPos(i: number, n: number, r: number): THREE.Vector3 {
  ...
  return new THREE.Vector3(r * Math.sin(phi) * Math.cos(theta), r * Math.cos(phi) * 0.7, r * Math.sin(phi) * Math.sin(theta));
}

/* Simple force-directed layout — runs synchronously for N iterations */
function forceLayout(nodes: { id: string; x: number; y: number; z: number }[], edges: GraphEdge[], iterations: number): void {
  ...
  for (let iter = 0; iter < iterations; iter++) {
    ...
    for (let i = 0; i < nodes.length; i++) {
      for (let j = i + 1; j < nodes.length; j++) {
        ...
        nodes[i].x += fx; nodes[i].y += fy; nodes[i].z += fz;
        nodes[j].x -= fx; nodes[j].y -= fy; nodes[j].z -= fz;
      }
    }
    ...
    for (const e of edges) {
      ...
      const dx = nodes[ti].x - nodes[si].x, dy = nodes[ti].y - nodes[si].y, dz = nodes[ti].z - nodes[si].z;
      const dist = Math.max(Math.sqrt(dx * dx + dy * dy + dz * dz), 0.1);
      const force = (dist / k) * temp;
      const fx = (dx / dist) * force, fy = (dy / dist) * force, fz = (dz / dist) * force;
      nodes[si].x += fx; nodes[si].y += fy; nodes[si].z += fz;
      nodes[ti].x -= fx; nodes[ti].y -= fy; nodes[ti].z -= fz;
    }
  }
}

export default function NeuralMap3D(props: NeuralMap3DProps): React.JSX.Element {
  const containerRef = useRef<HTMLDivElement>(null);
  const stateRef = useRef(props);
  const sceneRef = useRef<{ cleanup: () => void; rebuildGraph: () => void } | null>(null);
  ...
  const initScene = useCallback(() => {
    ...
    const bloom = new UnrealBloomPass(new THREE.Vector2(container.clientWidth, container.clientHeight), 1.4, 0.6, 0.3);
    ...
    for (let i = 0; i < dustCount; i++) {
      dustPos[i*3]=(Math.random()-.5)*30; dustPos[i*3+1]=(Math.random()-.5)*20; dustPos[i*3+2]=(Math.random()-.5)*30;
      dustVel[i*3]=(Math.random()-.5)*.003; dustVel[i*3+1]=(Math.random()-.5)*.002; dustVel[i*3+2]=(Math.random()-.5)*.003;
    }
    dustGeo.setAttribute("position", new THREE.BufferAttribute(dustPos, 3));
    const dustCloud = new THREE.Points(dustGeo, new THREE.PointsMaterial({ color: GREEN, size: 0.035, transparent: true, opacity: 0.35, blending: THREE.AdditiveBlending, depthWrite: false }));
    scene.add(dustCloud);
    ...
    interface SceneNode { mesh: THREE.Mesh; glow: THREE.Mesh; pos: THREE.Vector3; id: string; label: string; path?: string; }
    ...
    interface LabelEl { el: HTMLDivElement; pos: THREE.Vector3; id: string; }
    const labels: LabelEl[] = [];

    // Center brain (shared)
    const centerGeo = new THREE.IcosahedronGeometry(1.1, 3);
    const centerMat = new THREE.MeshPhysicalMaterial({ color: GREEN_DARK, emissive: GREEN, emissiveIntensity: 0.25, transparent: true, opacity: 0.18, roughness: 0.3, side: THREE.DoubleSide });
    const centerMesh = new THREE.Mesh(centerGeo, centerMat);
    scene.add(centerMesh);
    const wireGeo = new THREE.IcosahedronGeometry(1.15, 1);
    const wireMesh = new THREE.Mesh(wireGeo, new THREE.MeshBasicMaterial({ color: GREEN, wireframe: true, transparent: true, opacity: 0.22 }));
    scene.add(wireMesh);
    // inner cloud
    const ipGeo = new THREE.BufferGeometry();
    const ipPos = new Float32Array(150 * 3);
    for (let i = 0; i < 150; i++) { const r=Math.random()*0.9, t=Math.random()*Math.PI*2, p=Math.acos(2*Math.random()-1); ipPos[i*3]=r*Math.sin(p)*Math.cos(t); ipPos[i*3+1]=r*Math.sin(p)*Math.sin(t); ipPos[i*3+2]=r*Math.cos(p); }
    ipGeo.setAttribute("position", new THREE.BufferAttribute(ipPos, 3));
    const innerCloud = new THREE.Points(ipGeo, new THREE.PointsMaterial({ color: GREEN, size: 0.04, transparent: true, opacity: 0.7, blending: THREE.AdditiveBlending, depthWrite: false }));
    ...
    function buildNodes(): void {
      // Clear old
      for (const n of sceneNodes) { scene.remove(n.mesh); scene.remove(n.glow); n.mesh.geometry.dispose(); (n.mesh.material as THREE.Material).dispose(); n.glow.geometry.dispose(); (n.glow.material as THREE.Material).dispose(); }
      sceneNodes.length = 0;
      raycasterTargets.length = 0;
      for (const l of clusterBeams) { scene.remove(l); l.geometry.dispose(); (l.material as THREE.Material).dispose(); }
      clusterBeams.length = 0;
      for (const l of graphWebLines) { scene.remove(l); l.geometry.dispose(); (l.material as THREE.Material).dispose(); }
      ...
      if (isGraph && state.graphNotes && state.graphNotes.length > 0) {
        ...
        const edges = (state.graphEdges || []).filter(e => noteIdSet.has(e.source) && noteIdSet.has(e.target));

        // Force layout
        const layoutNodes = notes.map((n, i) => {
          const angle = (i / notes.length) * Math.PI * 2;
          const r = 4 + Math.random() * 4;
          return { id: n.id, x: Math.cos(angle) * r, y: (Math.random() - 0.5) * 4, z: Math.sin(angle) * r };
        });
        forceLayout(layoutNodes, edges, 80);
        const posMap = new Map(layoutNodes.map(n => [n.id, new THREE.Vector3(n.x, n.y, n.z)]));
        ...
        for (const note of notes) {
          ...
          const mat = new THREE.MeshPhysicalMaterial({ color: GREEN_DARK, emissive: GREEN_DIM, emissiveIntensity: note.linkCount > 0 ? 0.45 : 0.2, transparent: true, opacity: note.linkCount > 0 ? 0.75 : 0.5, roughness: 0.4, metalness: 0.15 });
          ...
          const glowMat = new THREE.MeshBasicMaterial({ color: GREEN, transparent: true, opacity: 0.07, blending: THREE.AdditiveBlending, depthWrite: false, side: THREE.BackSide });
          ...
          sceneNodes.push({ mesh, glow, pos, id: note.id, label: note.name.replace(/\.(md|markdown)$/i, ""), path: note.path });

          const el = document.createElement("div");
          el.className = "neural-3d-label neural-3d-label-clickable";
          el.textContent = note.name.replace(/\.(md|markdown)$/i, "").slice(0, 24);
        ...
        for (const edge of edges) {
          const sp = posMap.get(edge.source), tp = posMap.get(edge.target);
          if (!sp || !tp) continue;
          const lineGeo = new THREE.BufferGeometry().setFromPoints([sp, tp]);
          const lineMat = new THREE.LineBasicMaterial({ color: GREEN, transparent: true, opacity: 0.25, blending: THREE.AdditiveBlending, depthWrite: false });
        ...
        clusterNodes.forEach((node, i) => {
          ...
          const mat = new THREE.MeshPhysicalMaterial({ color: GREEN_DARK, emissive: GREEN_DIM, emissiveIntensity: node.value > 0 ? 0.35 : 0.08, transparent: true, opacity: node.value > 0 ? 0.65 : 0.25, roughness: 0.4 });
          const mesh = new THREE.Mesh(geo, mat);
          mesh.position.copy(pos);
          ...
          const glowMat = new THREE.MeshBasicMaterial({ color: GREEN, transparent: true, opacity: 0.06, blending: THREE.AdditiveBlending, depthWrite: false, side: THREE.BackSide });
          const glow = new THREE.Mesh(glowGeo, glowMat);
          glow.position.copy(pos);
          scene.add(glow);
          ...
          const lineMat = new THREE.LineBasicMaterial({ color: GREEN, transparent: true, opacity: 0.12, blending: THREE.AdditiveBlending, depthWrite: false });
          const line = new THREE.Line(lineGeo, lineMat);
          scene.add(line);
    ...
    function animate(): void {
      ...
      const graphKey = isGraph ? `${(state.graphNotes||[]).length}:${(state.graphEdges||[]).length}` : `c:${state.nodes.length}`;
      if (graphKey !== lastGraphKey) { lastGraphKey = graphKey; selectedGraphId = null; buildNodes(); }
      ...
      if (isGraph && search && search !== lastSearchApplied) {
        ...
        const match = sceneNodes.find(n => n.label.toLowerCase().includes(search));
        if (match?.path) {
      ...
      sceneNodes.forEach((entry, i) => {
        const isHit = intersects.length > 0 && intersects[0].object === entry.mesh;
        ...
        const matchesSearch = !search || entry.label.toLowerCase().includes(search);
        ...
        if (isHit) { hoveredId = entry.id; renderer.domElement.style.cursor = "pointer"; }

        const tgtEmissive = searchDim ? 0.03 : isActive ? 0.7 : isHit ? 0.5 : 0.2;
        const tgtOpacity = searchDim ? 0.1 : isActive ? 0.9 : isHit ? 0.8 : 0.5;
        const tgtScale = isActive ? 1.35 : isHit ? 1.2 : 1.0;
        const tgtGlow = searchDim ? 0.01 : isActive ? 0.18 : isHit ? 0.12 : 0.04;
        ...
        entry.mesh.scale.lerp(new THREE.Vector3(tgtScale, tgtScale, tgtScale), 0.08);
        entry.glow.scale.lerp(new THREE.Vector3(tgtScale, tgtScale, tgtScale), 0.06);
      ...
      for (let i = 0; i < dustCount; i++) {
        arr[i*3] += dustVel[i*3]*sm; arr[i*3+1] += dustVel[i*3+1]*sm; arr[i*3+2] += dustVel[i*3+2]*sm;
        if (Math.abs(arr[i*3])>15) arr[i*3]*=-0.9;
        if (Math.abs(arr[i*3+1])>10) arr[i*3+1]*=-0.9;
        if (Math.abs(arr[i*3+2])>15) arr[i*3+2]*=-0.9;
        if (isScanning) { arr[i*3]*=0.998; arr[i*3+1]*=0.998; arr[i*3+2]*=0.998; }
      }
      ...
      labels.forEach((lbl) => {
        ...
        const behind = p.z > 1;
        const searchMatch = !search || lbl.el.textContent!.toLowerCase().includes(search);
        const isSelected = isGraph && lbl.id === selectedGraphId;
        lbl.el.style.opacity = behind ? "0" : (isGraph && search && !searchMatch) ? "0.1" : isSelected ? "1" : "0.7";
        lbl.el.classList.toggle("active", isGraph ? isSelected : lbl.id === state.activeId);
      });
  ...
  return <div ref={containerRef} className="neural-3d-container" style={{ position: "absolute", inset: 0, overflow: "hidden", borderRadius: "inherit" }} />;
}
